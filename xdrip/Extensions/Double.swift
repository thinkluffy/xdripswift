...
extension Double: RawRepresentable {
    ...
    func mgdlToMmolAndToString(mgdl: Bool, withUnit: Bool = false) -> String {
        if mgdl {
            if withUnit {
                return String(format: "%.0f %@", self, Constants.bgUnitMgDl)
            ...
            if withUnit {
                return String(format: "%.1f %@", self.mgdlToMmol(), Constants.bgUnitMmol)
