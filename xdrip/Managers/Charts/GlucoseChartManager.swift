        ...
        let operation = BlockOperation(block: {
            ...
            if let lastGlucoseChartPoint = self.glucoseChartPoints.lastGlucoseChartPoint, let lastGlucoseChartPointX = lastGlucoseChartPoint.x as? ChartAxisValueDate {
                ...
                if startDateToUse > lastGlucoseTimeStamp {
                    ...
                    newGlucoseChartPointsToAppend = self.getGlucoseChartPoints(startDate: startDateToUse, endDate: endDate, bgReadingsAccessor: self.data().bgReadingsAccessor, on: CoreDataManager.shared.privateManagedObjectContext)
                    ...
                    newGlucoseChartPointsToAppend = self.getGlucoseChartPoints(startDate: lastGlucoseTimeStamp, endDate: endDate, bgReadingsAccessor: self.data().bgReadingsAccessor, on: CoreDataManager.shared.privateManagedObjectContext)
                ...
                if reUseExistingChartPointList {
                    ...
                    if let firstGlucoseChartPoint = self.glucoseChartPoints.firstGlucoseChartPoint, let firstGlucoseChartPointX = firstGlucoseChartPoint.x as? ChartAxisValueDate, startDateToUse < firstGlucoseChartPointX.date {
                        ...
                        newGlucoseChartPointsToPrepend = self.getGlucoseChartPoints(startDate: startDateToUse, endDate: firstGlucoseChartPointX.date, bgReadingsAccessor: self.data().bgReadingsAccessor, on: CoreDataManager.shared.privateManagedObjectContext)
            ...
            let newCalibrationChartPoints = self.getCalibrationChartPoints(startDate: startDateToUse, endDate: endDate, calibrationsAccessor: self.data().calibrationsAccessor, on: CoreDataManager.shared.privateManagedObjectContext)
...
//            yAxisValues += [ChartAxisValueDouble(UserDefaults.standard.urgentLowMarkValueInUserChosenUnit.bgValueRounded(mgdl: UserDefaults.standard.bloodGlucoseUnitIsMgDl), labelSettings: data().chartLabelSettingsObjectivesSecondary) as ChartAxisValue]
        ...
        // this will be the last default value of 300. If there is an objective "nearby" at > 280, then add the 300 guideline, but hide the label so that we don't crowd the axis
        if UserDefaults.standard.urgentHighMarkValueInUserChosenUnit <= (unitIsMgDl ? 280 : 15.5) {
            yAxisValues += [ChartAxisValueDouble((unitIsMgDl ? 300 : 16.6), labelSettings: data().chartLabelSettingsDimmed) as ChartAxisValue]
        } else if UserDefaults.standard.urgentHighMarkValueInUserChosenUnit < (unitIsMgDl ? 300 : 16.6) {
            yAxisValues += [ChartAxisValueDouble((unitIsMgDl ? 300 : 16.6), labelSettings: data().chartLabelSettingsHidden) as ChartAxisValue]
        }
        ...
        chartGuideLinesLayerSettings = ChartGuideLinesLayerSettings(linesColor: ConstantsGlucoseChart.gridColorObjectives,
                                                                    linesWidth: 1)
